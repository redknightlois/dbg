use super::{GpuDb, fmt_bytes, fmt_us, like_param, require_op_layer};
...
pub fn cmd_inspect(db: &GpuDb, args: &[&str]) {
    let pattern = match args.first() {
        Some(p) => *p,
        None => {
            println!("usage: inspect <kernel_pattern>");
            return;
        }
    };
    ...
    let sql = format!(
        r"SELECT kernel_name, COUNT(*), SUM(duration_us), AVG(duration_us),
    ...
    let config_sql = format!(
        "SELECT grid_x, grid_y, grid_z, block_x, block_y, block_z,
                             COUNT(*) as cnt
    ...
    let configs: Vec<(u32, u32, u32, u32, u32, u32, i64)> =
        db.query_vec(&config_sql, [&name], |row| {
            Ok((
                row.get(0)?,
                row.get(1)?,
                row.get(2)?,
    ...
    if let Ok(m) = db.conn.query_row(m_sql, [&name], |row| {
        ...
        if let Some(b) = &m.9 {
            println!("    Boundedness:       {b}");
        }
        if let Some(v) = m.0 {
            println!("    Occupancy:         {v:.1}%");
        }
        if let Some(v) = m.1 {
            println!("    Compute throughput: {v:.1}%");
        }
        if let Some(v) = m.2 {
            println!("    Memory throughput:  {v:.1}%");
        }
        if let Some(v) = m.3 {
            println!("    Registers/thread:  {v}");
        }
        let shmem = m.4.unwrap_or(0) + m.5.unwrap_or(0);
        if shmem > 0 {
            println!("    Shared memory:     {}", fmt_bytes(shmem));
        }
        if let Some(v) = m.6 {
            println!("    L2 hit rate:       {v:.1}%");
        }
        if let (Some(a), Some(p)) = (m.7, m.8) {
            println!(
                "    Bandwidth:         {a:.1} / {p:.1} GB/s ({:.1}%)",
                a / p * 100.0
            );
        } else if let Some(a) = m.7 {
            println!("    Bandwidth:         {a:.1} GB/s");
        }
    } else {
    ...
    let ops: Vec<(String, Option<String>, Option<String>)> = db.query_vec(op_sql, [&name], |row| {
...
pub fn cmd_trace(db: &GpuDb, args: &[&str]) {
    let pattern = match args.first() {
        Some(p) => *p,
        None => {
            println!("usage: trace <op_pattern>");
            return;
        }
    };
...
pub fn cmd_callers(db: &GpuDb, args: &[&str]) {
    let pattern = match args.first() {
        Some(p) => *p,
        None => {
            println!("usage: callers <kernel_pattern>");
            return;
        }
    };
    if !require_op_layer(db) {
    ...
    let rows: Vec<(String, Option<String>)> = db.query_vec(sql, [like_param(pattern)], |row| {
        Ok((row.get(0)?, row.get(1)?))
    });

    if rows.is_empty() {
        println!("no op mapping for kernels matching '{pattern}'");
        return;
    }
