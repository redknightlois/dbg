use super::{GpuDb, compute_gpu_gaps, fmt_bytes, fmt_us, parse_count, trunc, xfer_kernel_overlap};
...
pub fn cmd_transfers(db: &GpuDb, args: &[&str]) {
    if db.transfer_count() == 0 {
        ...
        if !db.has_layer("nsys") {
            println!("need nsys layer for transfer data");
        }
        return;
    }
    ...
    if wall_us > 0.0 {
        println!(
            "  {:.1}% of wall time spent on transfers",
            total_time / wall_us * 100.0
        );
    }
    ...
    if kernel_time > 0.0 {
        ...
        let verdict = if ratio > 5.0 {
            "BANDWIDTH-BOUND — PCIe dominates"
        } else if ratio > 1.5 {
            "transfer-heavy — consider async transfers or larger batches"
        } else if ratio > 0.5 {
            "mixed compute/transfer"
        } else {
            "compute-dominated"
        };
    ...
    for (kind, cnt, bytes, dur, min_b, max_b) in &kinds {
        let bw = if *dur > 0.0 {
            format!("{:.1} GB/s", *bytes as f64 / dur / 1000.0)
        } else {
            "?".into()
        };
        let range = if min_b == max_b {
            fmt_bytes(*min_b)
        } else {
            format!("{}-{}", fmt_bytes(*min_b), fmt_bytes(*max_b))
        };
        println!(
            "  {:<5} {:>8} {:>13} {:>11} {:>12} {}",
            kind,
    ...
    if small_cnt > 0 {
        let pct = if total_time > 0.0 {
            small_time / total_time * 100.0
        } else {
            0.0
        };
        println!(
            "  Small transfers: {} (<1 MB), {} total, {} time ({pct:.1}% of transfer time)",
            small_cnt,
            fmt_bytes(small_bytes),
            fmt_us(small_time)
        );
        if small_cnt > 10 {
    ...
    for (i, (kind, bytes, dur, sid)) in rows.iter().enumerate() {
        let bw_num = if *dur > 0.0 {
            *bytes as f64 / dur / 1000.0
        } else {
            0.0
        };
        let bw_str = if *dur > 0.0 {
            format!("{:.1} GB/s", bw_num)
        } else {
            "?".into()
        };
        ...
        let notes =
            if *bytes >= 16 * 1024 * 1024 && (kind == "H2D" || kind == "D2H") && bw_num < 6.0 {
...
pub fn cmd_gaps(db: &GpuDb, args: &[&str]) {
    ...
    if rows.is_empty() {
        println!("no GPU idle gaps detected");
        return;
    }
...
fn kernel_ending_at_or_before(db: &GpuDb, t: f64) -> Option<String> {
    ...
    db.conn
        .query_row(&sql, [t], |row| row.get::<_, String>(0))
        .ok()
}
...
pub fn cmd_overlap(db: &GpuDb) {
    ...
    if xfer_time > 0.0 && overlap_us > 0.0 {
        println!(
            "    Concurrent:        {} ({:.1}% of transfers overlapped with compute)",
            fmt_us(overlap_us),
            overlap_us / xfer_time * 100.0
        );
...
pub fn cmd_timeline(db: &GpuDb, args: &[&str]) {
    ...
    let rows: Vec<(String, f64, f64, Option<u32>)> = db.query_vec(&sql, [n as i64], |row| {
        Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?))
    });
