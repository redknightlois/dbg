...
pub(super) fn build_session() -> GpuDb {
    ...
    for (name, occ, cmp, mem, regs, ss, sd, l2, bw, peak, bound) in &metrics {
        db.conn
            .execute(
                ...
                params![
                    name, occ, cmp, mem, regs, ss, sd, l2, bw, peak, bound, ncu_id
                ],
            )
            .unwrap();
    }
    ...
    let linear_id: i64 = db
        .conn
        .query_row("SELECT id FROM ops WHERE name = 'aten::linear'", [], |r| {
            r.get(0)
        })
        .unwrap();
    let bn_id: i64 = db
        .conn
        .query_row(
            "SELECT id FROM ops WHERE name = 'aten::batch_norm'",
            [],
            |r| r.get(0),
        )
        .unwrap();
    let relu_id: i64 = db
        .conn
        .query_row("SELECT id FROM ops WHERE name = 'aten::relu_'", [], |r| {
            r.get(0)
        })
        .unwrap();
...
pub(super) fn build_triton_inference_session() -> GpuDb {
    ...
    let kernels = [
        (
            "triton_flash_attention_fwd_kernel",
            7_u32,
            vec![
    ...
    let attn_id: i64 = db
        .conn
        .query_row("SELECT id FROM ops WHERE name = 'attention'", [], |r| {
            r.get(0)
        })
        .unwrap();
    let linear_id: i64 = db
        .conn
        .query_row("SELECT id FROM ops WHERE name = 'linear'", [], |r| r.get(0))
        .unwrap();
    let ln_id: i64 = db
        .conn
        .query_row("SELECT id FROM ops WHERE name = 'layer_norm'", [], |r| {
            r.get(0)
        })
        .unwrap();
